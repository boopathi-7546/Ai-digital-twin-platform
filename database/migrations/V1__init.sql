...
-- This file is the Flyway-managed counterpart of ../schema.sql.
-- Backend uses Flyway (see application.yml: spring.flyway.enabled=true)
-- to apply this automatically on startup against an empty database.
--
-- Naming convention going forward:
--   V2__add_xyz.sql, V3__alter_abc.sql, etc.
-- Never edit this file after it has been applied to any environment —
-- add a new versioned migration instead.
-- =====================================================================

-- NOTE: Flyway runs against an already-selected schema (configured via
-- spring.datasource.url in application.yml), so this migration does NOT
-- include CREATE DATABASE / USE statements — unlike database/schema.sql,
-- which is meant for manual/local setup.
